...
data GameState = StartScreen | SierpinskiScreen Int | KochScreen Int | TreeScreen Int
...
drawState StartScreen = Pictures
    ...
    , Translate (-50) (-150) $ Scale 0.2 0.2 $ Text "Tree"
    ]
...
handleMainEvent (EventKey (MouseButton LeftButton) Down _ (x, y)) StartScreen
    ...
    | x >= (-50) && x <= 150 && y >= (-150) && y <= (-50) = TreeScreen 0
